import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import AppLayout from '@/layouts/app-layout';
import { Head } from '@inertiajs/react';
import { Award, Download, Trophy, CheckCircle2 } from 'lucide-react';

interface Course {
    id: number;
    title: string;
    slug: string;
    thumbnail: string | null;
    instructor: {
        name: string;
    };
    completed_at: string;
    quiz_score: number;
    quiz_total: number;
}

interface Props {
    completedCourses: Course[];
}

export default function CertificatesIndex({ completedCourses }: Props) {
    const getScorePercentage = (score: number, total: number) => {
        if (total === 0) return 0;
        return Math.round((score / total) * 100);
    };

    const getGradeColor = (percentage: number) => {
        if (percentage >= 90) return 'text-emerald-600 bg-emerald-50 border-emerald-600';
        if (percentage >= 80) return 'text-blue-600 bg-blue-50 border-blue-600';
        if (percentage >= 70) return 'text-indigo-600 bg-indigo-50 border-indigo-600';
        if (percentage >= 60) return 'text-amber-600 bg-amber-50 border-amber-600';
        return 'text-red-600 bg-red-50 border-red-600';
    };

    return (
        <AppLayout
            breadcrumbs={[
                { title: 'Dashboard', href: route('dashboard') },
                { title: 'Sertifikat', href: route('user.certificates.index') },
            ]}
        >
            <Head title="Sertifikat Saya" />

            <div className="flex flex-1 flex-col gap-6 p-6">
                {/* Header */}
                <div className="flex items-center gap-3">
                    <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-amber-500 to-orange-500">
                        <Award className="h-6 w-6 text-white" />
                    </div>
                    <div>
                        <h1 className="text-2xl font-semibold tracking-tight text-slate-900">
                            Sertifikat Saya
                        </h1>
                        <p className="text-sm text-slate-600">
                            Koleksi sertifikat dari kelas yang sudah kamu selesaikan.
                        </p>
                    </div>
                </div>

                {/* Stats */}
                {completedCourses.length > 0 && (
                    <div className="grid gap-4 md:grid-cols-3">
                        <Card className="border border-blue-200 bg-blue-50">
                            <CardContent className="flex items-center gap-4 p-4">
                                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-blue-600">
                                    <Trophy className="h-5 w-5 text-white" />
                                </div>
                                <div>
                                    <p className="text-xs font-medium uppercase tracking-wide text-blue-900/80">
                                        Total Sertifikat
                                    </p>
                                    <p className="text-2xl font-bold text-blue-600">
                                        {completedCourses.length}
                                    </p>
                                </div>
                            </CardContent>
                        </Card>
                    </div>
                )}

                {/* Certificates Grid */}
                {completedCourses.length > 0 ? (
                    <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                        {completedCourses.map((course) => {
                            const percentage = getScorePercentage(course.quiz_score, course.quiz_total);
                            const gradeColor = getGradeColor(percentage);

                            return (
                                <Card
                                    key={course.id}
                                    className="group overflow-hidden border border-slate-200 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg"
                                >
                                    <CardContent className="p-0">
                                        {/* Thumbnail */}
                                        <div className="relative h-40 overflow-hidden bg-gradient-to-br from-blue-500 to-indigo-600 sm:h-48">
                                            {course.thumbnail ? (
                                                <img
                                                    src={course.thumbnail}
                                                    alt={course.title}
                                                    className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-110"
                                                />
                                            ) : (
                                                <div className="flex h-full items-center justify-center">
                                                    <Award className="h-12 w-12 text-white opacity-60" />
                                                </div>
                                            )}
                                            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
                                            <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between">
                                                <Badge className="bg-white/90 text-emerald-600 backdrop-blur-sm">
                                                    <CheckCircle2 className="mr-1 h-3 w-3" />
                                                    Selesai
                                                </Badge>
                                            </div>
                                        </div>

                                        {/* Content */}
                                        <div className="space-y-4 p-5">
                                            <div>
                                                <h3 className="mb-1 line-clamp-2 text-base font-semibold text-slate-900 sm:text-lg">
                                                    {course.title}
                                                </h3>
                                                <p className="text-xs text-slate-500 sm:text-sm">
                                                    Instruktur: {course.instructor.name}
                                                </p>
                                            </div>

                                            {/* Score */}
                                            <div className={`rounded-lg border px-3 py-2.5 text-sm ${gradeColor}`}>
                                                <div className="flex items-center justify-between gap-3">
                                                    <div>
                                                        <p className="text-[11px] font-medium uppercase tracking-wide opacity-80">
                                                            Skor Quiz
                                                        </p>
                                                        <p className="text-lg font-bold">
                                                            {course.quiz_score} / {course.quiz_total}
                                                        </p>
                                                    </div>
                                                    <div className="text-right">
                                                        <p className="text-[11px] font-medium uppercase tracking-wide opacity-80">
                                                            Persentase
                                                        </p>
                                                        <p className="text-lg font-bold">
                                                            {percentage}%
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>

                                            {/* Completion Date */}
                                            <div className="flex items-center justify-between text-xs text-slate-500 sm:text-sm">
                                                <span>
                                                    Selesai: {course.completed_at}
                                                </span>
                                            </div>

                                            {/* Download Button */}
                                            <Button
                                                asChild
                                                className="w-full bg-gradient-to-r from-amber-500 to-orange-500 text-sm font-semibold hover:from-amber-600 hover:to-orange-600"
                                            >
                                                <a
                                                    href={route('user.certificates.download', { slug: course.slug })}
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                >
                                                    <Download className="mr-2 h-4 w-4" />
                                                    Download Sertifikat
                                                </a>
                                            </Button>
                                        </div>
                                    </CardContent>
                                </Card>
                            );
                        })}
                    </div>
                ) : (
                    <Card className="border border-dashed">
                        <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                            <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-full bg-slate-100">
                                <Award className="h-10 w-10 text-slate-400" />
                            </div>
                            <h3 className="mb-2 text-xl font-bold text-slate-900">
                                Belum Ada Sertifikat
                            </h3>
                            <p className="mb-6 max-w-md text-slate-600">
                                Kamu belum menyelesaikan kelas apa pun. Selesaikan semua video dan quiz untuk mendapatkan sertifikat!
                            </p>
                            <Button
                                asChild
                                className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700"
                            >
                                <a href={route('user.learn')}>
                                    Mulai Belajar
                                </a>
                            </Button>
                        </CardContent>
                    </Card>
                )}
            </div>
        </AppLayout>
    );
}
